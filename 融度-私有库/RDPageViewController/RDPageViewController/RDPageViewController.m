...
@interface RDPageViewController () <UIPageViewControllerDelegate, UIPageViewControllerDataSource, UIScrollViewDelegate>
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    self = [[RDPageViewController alloc] initWithTransitionStyle:UIPageViewControllerTransitionStyleScroll
...
- (void)reloadPagesToCurrentPageIndex:(NSInteger)currentPageIndex {
    ...
    [self connectButtons];
    if([[self RDDataSource] respondsToSelector:@selector(otherConfiguration)]) {
        [[self RDDataSource] otherConfiguration];
    }
}
...
{
    NSAssert([[self RDDataSource] isKindOfClass:[UIViewController class]], @"This needs to be implemented in a class that inherits from UIViewController");
    [(UIViewController *)[self RDDataSource] addChildViewController:self];
    [[[self RDDataSource] RDPageContainer] addSubview:self.view];
}

- (void)loadControllers
{
    [_viewControllerArray removeAllObjects];
    NSArray* controllers = [[self RDDataSource] RDPageControllers];
...
- (void)connectButtons
{
    NSArray* buttons = [[self RDDataSource] RDPageButtons];
    NSAssert([buttons count] > 0, @"Buttons needs to be at least 1");
    
    if (_pageMode == RD_SegmentController) {
...
//        NSAssert(segmentController.numberOfSegments == [_viewControllerArray count], @"The number of segments needs to be the same as the number of controllers");
    ...
    for (UIButton* button in buttons){
        NSAssert([button isKindOfClass:[UIButton class]], @"Add buttons to RDPageButtons Array need to contain only UIButton elements");
        button.tag = i;
...
{
    UIView* container = [[self RDDataSource] RDPageContainer];
...
- (void)setPageControllerForIndex:(NSInteger)index direction:(UIPageViewControllerNavigationDirection)direction currentRDViewController:(id)weakSelf destionation:(NSInteger)destination
{
    ...
    [_pageController setViewControllers:@[[_viewControllerArray objectAtIndex:index]] direction:direction animated:NO completion:^(BOOL complete){
        ...
        if (complete && strongSelf) {
            if ((pageModeBlock == RD_SegmentController) && (index != destination)) return; // It should update the segment buttons when the user already pressed in a segment
...
{
    if ([self RDDataDelegate] && [[self RDDataDelegate] respondsToSelector:@selector(RDPageChangedToIndex:)]) {
        [[self RDDataDelegate] RDPageChangedToIndex:index];
    }
